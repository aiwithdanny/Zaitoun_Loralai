...
- After code analysis narrows a bug down to multiple possible causes, run a live reproduction test to confirm which cause is real — capture the actual HTTP status code, response body, and database state, then report the real evidence before proposing any fix. Do not present speculation as likely scenarios without confirmation. Confidence: 0.70
...
- After implementing approved changes, commit and push to the remote repository (git add, git commit, git push) — do not leave changes sitting uncommitted locally. Confidence: 0.70
