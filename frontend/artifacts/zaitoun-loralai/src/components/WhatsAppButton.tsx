...
const FALLBACK_NUMBER = "923492882897";
const FALLBACK_MESSAGE = "Hi, I'm interested in Zaitoun Loralai products.";

export function WhatsAppButton() {
  const [location] = useLocation();
  const [whatsappNumber, setWhatsappNumber] = useState(FALLBACK_NUMBER);
  const [whatsappMessage, setWhatsappMessage] = useState(FALLBACK_MESSAGE);

  useEffect(() => {
    wholesaleApi.getActive().then((data) => {
      if (data.config?.whatsapp_number) setWhatsappNumber(data.config.whatsapp_number);
      if (data.config?.whatsapp_message) setWhatsappMessage(data.config.whatsapp_message);
    }).catch(() => {});
  }, []);
